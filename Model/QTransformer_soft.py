...
class SQARHead(nn.Module):
    """
    将 decoder_out 转成 SQAR 需要的多头输出：
      - A-head: 两套 (ball / players) 的 bin logits（未做 soft 归一）
      - V-head: 两套标量 V(s)（用 decoder_out 的 pooled 表征）
    输入:
      decoder_out: Tensor, [S, B*T, D]，S=12(含P/A交替) 或 S=6(已取P位)
                   当 S=12 时，本模块内部会用 decoder_out[::2] 取出 6 个 P 位：
                      位置0=ball，1~5=players
    返回:
      out = {
        "A": {
          "ball": {"1": q1_ball_logits, "2": q2_ball_logits},   # [B*T, 1, C_ball]
          "pl":   {"1": q1_pl_logits,   "2": q2_pl_logits},     # [B*T, 5, C_pl]
        },
        "V": {"1": v1_bt, "2": v2_bt},                           # [B*T, 1]
        "nbias": nb_bt or None                                   # [B*T, 1] / None
      }
    """
    def __init__(
        self,
        ...
        num_bins_ball: int,
        num_bins_player: int,
        use_nbias: bool = False,
        init_kaiming: bool = True,
    ):
        super().__init__()
        self.D = embed_dim
        self.C_ball = num_bins_ball
        self.C_pl = num_bins_player
        self.use_nbias = use_nbias

        # A-heads（未做 soft）
        self.qball_head_1 = nn.Sequential(
            nn.Linear(self.D, self.D), nn.LeakyReLU(inplace=True),
            nn.Linear(self.D, self.C_ball)
        )
        self.qball_head_2 = nn.Sequential(
            nn.Linear(self.D, self.D), nn.LeakyReLU(inplace=True),
            nn.Linear(self.D, self.C_ball)
        )
        self.qpl_head_1 = nn.Sequential(
            nn.Linear(self.D, self.D), nn.LeakyReLU(inplace=True),
            nn.Linear(self.D, self.C_pl)
        )
        self.qpl_head_2 = nn.Sequential(
            nn.Linear(self.D, self.D), nn.LeakyReLU(inplace=True),
            nn.Linear(self.D, self.C_pl)
        )

        # V-heads（两套）
        self.vball_head_1 = nn.Sequential(
            nn.Linear(self.D, self.D // 2), nn.ReLU(),
            nn.Linear(self.D // 2, 1)
        )
        self.vball_head_2 = nn.Sequential(
            nn.Linear(self.D, self.D // 2), nn.ReLU(),
            nn.Linear(self.D // 2, 1)
        )
        self.vpl_head_1 = nn.Sequential(
            nn.Linear(self.D, self.D // 2), nn.ReLU(),
            nn.Linear(self.D // 2, 1)
        )
        self.vpl_head_2 = nn.Sequential(
            nn.Linear(self.D, self.D // 2), nn.ReLU(),
            nn.Linear(self.D // 2, 1)
        )
        ...
        # 可选 nbias
        if self.use_nbias:
            self.nbias_head = nn.Sequential(
                nn.Linear(self.D, self.D // 2), nn.ReLU(),
                nn.Linear(self.D // 2, 1)
            )
        else:
            self.nbias_head = None

        if init_kaiming:
            _init_linear_kaiming(self)
    def forward(self, decoder_out: torch.Tensor):
        """
        decoder_out: [S, B*T, D],  S=12(含P/A交替) 或 S=6(仅P位)
        约定：P位顺序 [ball, p1, p2, p3, p4, p5]
        """
        S, BT, D = decoder_out.shape
        assert D == self.D, f"decoder_out hidden dim {D} != expected {self.D}"

        # 取出 6 个 P 位 token: [6, B*T, D]
        if S == 12:
            p_tokens = decoder_out[::2, :, :]
        elif S == 6:
            p_tokens = decoder_out
        else:
            raise ValueError(f"Unexpected decoder_out length S={S}. Expect 12 (P/A alternation) or 6 (P only).")

        # 切分 ball 与 players
        ball_tok = p_tokens[0:1, :, :]     # [1, B*T, D]
        pl_tok   = p_tokens[1: , :, :]     # [5, B*T, D]

        # ---- A-head logits（未 soft）----
        q1_ball_logits = self.qball_head_1(ball_tok)   # [1, B*T, C_ball]
        q2_ball_logits = self.qball_head_2(ball_tok)   # [1, B*T, C_ball]
        q1_pl_logits   = self.qpl_head_1(pl_tok)       # [5, B*T, C_pl]
        q2_pl_logits   = self.qpl_head_2(pl_tok)       # [5, B*T, C_pl]

        # reshape 到 [B*T, *, C]
        q1_ball_logits = rearrange(q1_ball_logits, 'n bt c -> bt n c')
        q2_ball_logits = rearrange(q2_ball_logits, 'n bt c -> bt n c')
        q1_pl_logits   = rearrange(q1_pl_logits,   'n bt c -> bt n c')
        q2_pl_logits   = rearrange(q2_pl_logits,   'n bt c -> bt n c')

        # ---------------- V-head（四个头，对称） ----------------
        # ball V: 对 ball token
        v1_ball = self.vball_head_1(ball_tok)                  # [1, B*T, 1]
        v2_ball = self.vball_head_2(ball_tok)                  # [1, B*T, 1]
        v1_ball = rearrange(v1_ball, 'n bt d -> bt n d')       # [B*T,1,1]
        v2_ball = rearrange(v2_ball, 'n bt d -> bt n d')       # [B*T,1,1]

        # players V: 对 5 个 player token，逐个输出
        v1_pl = self.vpl_head_1(pl_tok)                        # [5, B*T, 1]
        v2_pl = self.vpl_head_2(pl_tok)                        # [5, B*T, 1]
        v1_pl = rearrange(v1_pl, 'n bt d -> bt n d')           # [B*T,5,1]
        v2_pl = rearrange(v2_pl, 'n bt d -> bt n d')           # [B*T,5,1]
        

        nb_bt = self.nbias_head(pooled_bt) if self.nbias_head is not None else None  # [B*T,1] / None
        ...
        out = {
            ...
            "V": {
                "ball": {"1": v1_ball, "2": v2_ball},  # [B*T,1,1]
                "pl":   {"1": v1_pl,               "2": v2_pl},                 # [B*T,5,1]
            },
            "nbias": nb_bt
        }
        return out
...
class QTransformer(nn.Module):
    def __init__(self, cfg):
        ...
        self.sqar_head = SQARHead(self.embed_dim, self.num_bins_ball,self.num_bins_player , use_nbias=False, init_kaiming=True)
    ...
    def forward(self, state_tokens, agent_ids, padding_mask, action_tokens,edge_index,edge_attr):
       ...
       # print(action_tokens_.dtype)
        ...
        #decoder_out [n,b,d] n=12 b=B*T d=128
        sqar_out = self.sqar_head(decoder_out)
        ...
        out = {
                "A": sqar_out["A"],
                "V": sqar_out["V"],        # 现在是四头结构：ball.{1,2}, pl.{1,2}
                "nbias": sqar_out["nbias"],
                "qsq": qsq
            }
