...
from .QTransformer_soft_pointer_decoder import QTransformer
...
class QT(pl.LightningModule):
    ...
    def forward(self, batch):
        ...
        action_tokens = batch['action_tokens'] #  #30为无效帧补齐的

        edge_index = batch['edge_index']  #[2,110]
        edge_attr = batch['new_edge_attr'] #[B,T,110,3]
    ...
    def training_step(self, batch, batch_idx):
        total_loss,td_loss,cql_loss,entropy_mean,qsq_loss,kl_loss,ce_loss = self.compute_loss(batch)
    ...
    def compute_loss(self, batch):
        ...
        #rewards  = batch['rewards']               # [B, T]
        rewards  = batch['rewards_quantile']      # [B, T]
        done     = batch['done']                  # [B]
        mc_ret   = batch['mc_return']             # [B, T]
        edge_idx = batch['edge_index']            # [2,132]
        edge_attr= batch['new_edge_attr']             # [B,T,132,3]
        ...
        a_ball_1 = a_ball_1 - a_ball_1.mean(dim=-1, keepdim=True)
        a_ball_2 = a_ball_2 - a_ball_2.mean(dim=-1, keepdim=True)

        a_pl_1 = a_pl_1 - a_pl_1.mean(dim=-1, keepdim=True)
        a_pl_2 = a_pl_2 - a_pl_2.mean(dim=-1, keepdim=True)
        ...
        q_pred_1_ball = v_pred + adv1a_ball.unsqueeze(-1)          # [B*T,1,1]
        q_pred_2_ball = v_pred + adv2a_ball.unsqueeze(-1)         # [B*T,1,1]

        q_pred_1_player =  v_pred.expand(-1,5,-1)  + adv1a_pl.unsqueeze(-1)    # [B*T,5,1]
        q_pred_2_player =  v_pred.expand(-1,5,-1)  + adv2a_pl.unsqueeze(-1)
        ...
        with torch.no_grad():
            ...
            V_target = out_target['V'].unsqueeze(-1) #[ B*T,1, 1]
           ...
           #-------------------------------------------
            q_pl_target_1 = V_target.expand(-1,5,-1) + a_target_pl_1   # [B*T,5,19]
            q_pl_target_2 = V_target.expand(-1,5,-1) + a_target_pl_2
            ...
            q_ball_target_1 = V_target + a_target_ball_1    # [B*T,1,6]
            q_ball_target_2 = V_target + a_target_ball_2    # [B*T,1,6]
            ...
            #q_ball_target = torch.min(q_ball_target_1_,q_balL_target_2_)  #[ B*T,1, 1] 
            #print("q_ball_target after min:", q_ball_target.shape)
        ...
        # batch_idx = torch.arange(B, device=rewards.device)
        # for b in range(B):
        #     term_idx = done[b] - 1  # 最后一个有效 step 的 index
        #     #term_idx = T - 1
        #     print(patched_q_target[b, term_idx+1, 0])
        #     print(td_target_ball[b, term_idx])
        #     print(rewards[b, term_idx])
        ...
        if self.only_ball:
            ...
            pass_mask = (ball_action[:,1:] != 6) & (ball_action[:,1:] != 30)  # [B,T-1] 传球动作且有效帧
            ...
            td_loss_pass = td_loss_ball # td loss for pass and shot
        ...
        # pred ball ： q_bal_1 a_ball_2 [B*T,1,7]
        ...
        q_ball_1 = v_pred  + a_ball_1      # [B*T, 1, 6]
        q_ball_2 = v_pred  + a_ball_2      # [B*T, 1, 6]  
        ...
        q_pl_1 = v_pred.expand(-1,5,-1) + nb + a_pl_1      # [B*T, 5, 19]
        q_pl_2 = v_pred.expand(-1,5,-1)  + nb + a_pl_2      # [B*T, 5, 19]
        ...
        if self.only_ball:
            ...
            cql_loss = 0.5 * ( (cql_loss_ball_1.mean() + cql_loss_ball_2.mean())/2 
        ...
        #action_logits  # [B*T, 2]
        valid_mask = (ball_action != 30).reshape(-1)  # [B*T]
        
        # action_label_ = action_label[valid_mask]
        # num_total = action_label_.numel()
        # num_0 = (action_label_ == 0).sum().item()
        # num_1 = (action_label_ == 1).sum().item()

        # print(f"Total samples: {num_total}")
        # print(f"no-dribble (0): {num_0} ({num_0 / num_total:.3%})")
        # print(f"dribble (1): {num_1} ({num_1 / num_total:.3%})")
        
        ce_loss_frame = F.cross_entropy(
            action_logits[valid_mask],   # 预测: [N_valid, 2]
            action_label[valid_mask],     # 标签: [N_valid]
            weight=torch.tensor([13.0, 1.0], device=action_logits.device),  # 给类别加权，鼓励识别传球
            reduction='none'
        )
        ...
        if self.global_step < warmup_steps:
            total_loss = qsq_loss * 1.0 + 1.0 * ce_loss  # 仅优化 qSQ
        else:
            total_loss = td_loss + 0.5 * cql_loss + 0.1 * qsq_loss + 1.0 * ce_loss #- 0.0 * entropy_mean + 0.0 * kl_loss
        ...
        if self.global_step % 50 == 0:
            with torch.no_grad():
                ...
                # ✅ 每个 action bin 的 Q 值分布
                num_bins = q_ball_pred.shape[-1]
                for bin_idx in range(num_bins):
                    q_bin_values = valid_q_ball[:, bin_idx]  # [N]
                    self.logger.experiment.add_histogram(
                        tag=f"q_values/ball1_bin{bin_idx}",
                        values=q_bin_values,
                        global_step=self.global_step
                    )
        ...
        return total_loss,td_loss,cql_loss,entropy_mean,qsq_loss,kl_loss,ce_loss
